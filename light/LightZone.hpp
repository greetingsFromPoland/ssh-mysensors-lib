...
public:
    ...
    LightZone(const VurtualSensorData &sensorData, Lamp *lamps[], uint8_t lampsSize);

    void turnOn(); // włącza
    void turnOff();

    void toggle(); // zmiania stan na przeciwny
    void turnOnForTime(uint8_t milis); // włącza na określony czas
    void run(); // uruchamiane przy kadym loop()
    void setup(); // konfiguracja pinow arduino
    void flash(uint8_t repeatNumber, uint8_t durationInMilis); // miganie
...
LightZone::LightZone(const VurtualSensorData &sensorData, Lamp *lamps[], uint8_t lampsSize) {
